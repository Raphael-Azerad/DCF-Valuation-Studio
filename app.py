...
with st.sidebar:
    ...
    ticker_choice = st.selectbox("Ticker", ["JPM", "AAPL", "LMT", "MSFT", "NVDA", "Custom"], index=0)
    ticker = st.text_input("Custom ticker", value="JPM" if ticker_choice != "Custom" else "").upper()
    if ticker_choice != "Custom":
        ticker = ticker_choice
    run_button = st.button("Run Valuation", width="stretch")

if "active_ticker" not in st.session_state:
    st.session_state.active_ticker = "JPM"
...
with st.sidebar:
    st.divider()
    st.caption("Market data overrides")
    ...
    st.divider()
    st.caption("Forecast assumptions")
