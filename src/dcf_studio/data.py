...
def is_financial_institution(sector: str | None, industry: str | None = None) -> bool:
    """Return True when a ticker is likely a bank, insurer, or other financial institution."""
...
def fetch_company_data(ticker: str) -> FinancialDataset:
    ...
    if is_financial_institution(overview.sector, overview.industry):
        warnings.append(
            "Financial Institution Warning: banks, insurers, and other financial institutions do not usually fit "
            "a standard enterprise-value free-cash-flow DCF cleanly. Treat this output as an educational "
            "normalization exercise and review FCF, cash, debt, and regulatory-capital assumptions carefully."
        )
