...
def test_calculate_dcf_outputs_intrinsic_value():
    ...
    assert result.pv_of_forecast_cash_flows == pytest.approx(result.forecast_pv["PV Free Cash Flow"].sum())
    assert result.equity_value == pytest.approx(result.enterprise_value - result.debt + result.cash)
...
def test_dcf_rejects_zero_shares_outstanding():
    with pytest.raises(ValuationInputError, match="Shares outstanding"):
        calculate_dcf(
            DCFInputs(
                forecast=forecast_frame(),
                discount_rate=0.10,
